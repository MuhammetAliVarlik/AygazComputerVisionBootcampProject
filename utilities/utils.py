...
def get_manipulated_images(image):
    ...
    return [cv2.cvtColor((manipulated_purplish*255).astype(np.uint8), cv2.COLOR_RGB2BGR),
            cv2.cvtColor((manipulated_yellowish*255).astype(np.uint8), cv2.COLOR_RGB2BGR),
            cv2.cvtColor((manipulated_greenish*255).astype(np.uint8), cv2.COLOR_RGB2BGR)]
